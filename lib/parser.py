...
class Parser:
    ...
    def parse_statement(self):
        ...
        elif self.peek_ahead(1).type == TokenType.EQUALS:
            return self.parse_equals_assign()
        else:
            try:
    ...
    def parse_equals_assign(self):
        name_token = self.advance()  # consume variable name
    ...
    def parse_primary(self):
        ...
        if token.type == TokenType.STRING:
            self.advance()
            return self._parse_post_primary(Literal(token.value))

        if token.type == TokenType.NUMBER:
            self.advance()
            if "." in token.value:
                return self._parse_post_primary(Literal(float(token.value)))
            return self._parse_post_primary(Literal(int(token.value)))
        ...
        if token.type == TokenType.LPAREN:
            ...
            return self._parse_post_primary(expr)

        if token.type == TokenType.WORD:
            word = token.value.lower()
            if word == "true":
                self.advance()
                return self._parse_post_primary(Literal(True))
            elif word == "false":
                self.advance()
                return self._parse_post_primary(Literal(False))
            elif word == "run":
                return self._parse_post_primary(self.parse_run_expression())
            else:
                self.advance()
                return self._parse_post_primary(VariableReference(token.value))

        raise ParseError(f"Unexpected token: {token.value!r}", token)

    def _parse_post_primary(self, expr):
        while True:
            if self.peek().type == TokenType.PERIOD:
                ...
                if self.peek().type == TokenType.LPAREN:
                    ...
                    if self.peek().type != TokenType.RPAREN:
                        ...
                        while self.peek().type == TokenType.COMMA:
                            ...
                            arguments.append(self.parse_expression())
                    self.expect(TokenType.RPAREN)
                    expr = MethodCall(expr, name, arguments)
                else:
                    expr = MemberAccess(expr, name)
            elif self.peek().type == TokenType.LPAREN and isinstance(expr, VariableReference):
                self.advance()  # consume '('
                arguments = []
                if self.peek().type != TokenType.RPAREN:
                    arguments.append(self.parse_expression())
    ...
    def parse_dict_literal(self):
        ...
        if self.peek().type != TokenType.RBRACE:
            ...
            elif self.peek().type == TokenType.WORD:
                key_token = self.advance()
                keys.append(key_token.value)
            else:
                raise ParseError(f"Expected key in dictionary", self.peek())
    ...
    def parse_list_literal(self):
        ...
        return ListDeclaration("", elements)
    ...
    def parse_import(self):
        ...
        filepath_token = self.expect(TokenType.STRING)
        if self.peek().type == TokenType.PERIOD:
            self.advance()
        return ImportStatement(filepath_token.value)
